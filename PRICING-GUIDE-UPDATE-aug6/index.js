  ...
  if (n === 1 && clientId) {
    const guideChain = (async () => {
      try {
        ...
        if (firstTime && settingsG.booking_link && c.env.GHL_TOKEN && settingsG.ghl_location_id) return; // booking email carries the guide — no double email
    ...
    if (settings.booking_link && c.env.GHL_TOKEN && settings.ghl_location_id) {
      const sendBooking = (async () => {
        try {
          ...
          // 💼 (8/6) the guide rides WITH the booking email — pricing lands before the
          // call, in one smooth email instead of two. They arrive at the call pre-sold.
          let guideBlock = '';
          if (clientId) {
            try {
              const gurlB = `${BASE_URL}/guide/${clientId}/${await portalToken(c.env, 'guide', clientId)}`;
              guideBlock = `<p><b>Before we talk:</b> here is your personalized packages guide. Both builds, what each includes, and exactly how the pricing works, so our call can focus on your plan:</p><p><a href="${gurlB}"><b>View your guide</b></a></p>`;
            } catch { /* guide link is a bonus — booking email must still go */ }
          }
          await ghl.sendEmail({
            contactId,
            subject: renderTemplate(settings.booking_subject, { name: firstName }),
            html: renderTemplate(settings.booking_body, { name: firstName, booking_link: settings.booking_link }) + guideBlock,
            emailFrom: settings.email_from || undefined,
          });
          await logEvent(db, clientId, 'booking_email_sent', `Booking link sent to ${email}`);
          if (clientId && guideBlock) {
            try {
              const clB = await db.prepare('SELECT * FROM clients WHERE id = ?').bind(clientId).first();
              const bB = getBilling(clB); bB.guide_sent = new Date().toISOString();
              await touchClient(db, clientId, { billing: JSON.stringify(bB) });
              await logEvent(db, clientId, 'guide_sent', `💼 Pricing guide included with the booking email`);
